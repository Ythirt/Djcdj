...
#mizaaaaat
             ...
             #/inv                   #spam_invite
                                if '1200' in dataS.hex()[0:4] and '2f696e76' in dataS.hex()[0:900] : 
                                   ...
                                   #/inf 
                                    #invite_spam OFF
                                if '1200' in dataS.hex()[0:4] and '2f696e66' in dataS.hex()[0:900] :
                                    inviteD =False
                      ...
                      #/v                                      
                                if '1200' in dataS.hex()[0:4] and '2f76' in dataS.hex()[0:900] :
                                    self.spam_level=True
                                    

                                    
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FF00][b][c]LVL UP")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FF00][b][c]LVL UP"))))
                                      
                                    
                                #lvl off
                                if '1200' in dataS.hex()[0:4] and '2f7566' in dataS.hex()[0:900] :
                                

                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[FF0000][b][c] LVL OFF")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[FF0000][b][c]LVL OFF"))))
...
#           /5
                                if '1200' in dataS.hex()[0:4] and '2f35' in dataS.hex()[0:900]:
                                    client.send(bytes.fromhex(gen_msgv2(dataS.hex() ,"[00FFFF][b][c]5 in Group ")))
                                    client.send(bytes.fromhex(str(gen_msgv2_clan(dataS.hex() ,"[00FFFF][b][c] 5 in Group"))))
